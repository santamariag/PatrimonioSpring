package it.poste.patrimonio.batch.bl.util;

public class BatchConstants {
    public static final String LOAD_CSV_FILE_JOB = "loadCsvPriceFileJob";
    public static final String LOAD_CSV_STEP_PARTITIONER = "loadCsvStepPartitioner";
    public static final String THREAD_NAME_PREFIX = "spring_batch";
    public static final String SLAVE_THREAD = " Slave_Thread_";
    public static final String LOAD_CSV_STEP = "loadCsvStep";
    
    public static final String LOAD_CSV_MFM_FILE_JOB = "loadCsvMfmFileJob";
    public static final String LOAD_CSV_MFM_STEP = "loadCsvMfmStep";
    public static final String LOAD_AFB_STEP = "loadFixedAfbStep";
    public static final String LOAD_AFB_FILE_JOB = "loadFixedAfbFileJob";
}